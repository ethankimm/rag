...
chapter: "chapter3"
original_path: "chapters/en/chapter3/4.mdx"
source_revision: "5805d51523d561a82520b301dbc8c5759b212844"
source_url: "https://github.com/huggingface/course/blob/5805d51523d561a82520b301dbc8c5759b212844/chapters/en/chapter3/4.mdx"
---
# A full training loop


[Course video](https://www.youtube.com/watch?v=Dh9CL8fyG80)
...
[Course video](https://www.youtube.com/watch?v=s7dy8QRgjJ0)
...
**Choices**
- AdamW uses a different learning rate schedule.
  - Explanation: Learning rate scheduling is separate from the optimizer choice.
- AdamW includes decoupled weight decay regularization.
  - Explanation: Correct! AdamW separates weight decay from the gradient-based parameter updates, leading to better regularization.
- AdamW only works with transformer models.
  - Explanation: AdamW can be used with any model architecture, not just transformers.
- AdamW requires less memory than Adam.
  - Explanation: Both optimizers have similar memory requirements.

### 2. In a training loop, what is the correct order of operations?

**Choices**
- Forward pass → Backward pass → Optimizer step → Zero gradients
  - Explanation: Close, but you should zero gradients before the next forward pass to avoid accumulating old gradients.
- Forward pass → Backward pass → Optimizer step → Scheduler step → Zero gradients
  - Explanation: Correct! This is the proper order: compute loss, compute gradients, update parameters, update learning rate, then clear gradients.
- Zero gradients → Forward pass → Optimizer step → Backward pass
  - Explanation: The backward pass must come after the forward pass to compute gradients from the loss.
- Forward pass → Zero gradients → Backward pass → Optimizer step
  - Explanation: Zeroing gradients before backward pass would eliminate the gradients you just computed.

### 3. What does the 🤗 Accelerate library primarily help with?

**Choices**
- Making your models train faster by optimizing the forward pass.
  - Explanation: Accelerate doesn't optimize the model architecture itself.
- Automatically selecting the best hyperparameters.
  - Explanation: Accelerate doesn't do hyperparameter optimization.
- Enabling distributed training across multiple GPUs/TPUs with minimal code changes.
  - Explanation: Correct! Accelerate handles distributed training complexity, allowing your code to run on single or multiple devices seamlessly.
- Converting models to different frameworks like TensorFlow.
  - Explanation: Accelerate works within PyTorch and doesn't convert between frameworks.

### 4. Why do we move batches to the device in a training loop?

**Choices**
- To make the training faster.
  - Explanation: While it can affect speed, the main reason is compatibility.
- Because the model and data must be on the same device (CPU/GPU) for computation.
  - Explanation: Correct! PyTorch requires tensors to be on the same device for operations to work.
- To save memory.
  - Explanation: Moving to device doesn't inherently save memory.
- It's required by the DataLoader.
  - Explanation: DataLoader doesn't require specific device placement.

### 5. What does `model.eval()` do before evaluation?

**Choices**
- It freezes the model parameters so they can't be updated.
  - Explanation: model.eval() doesn't freeze parameters - that would be done by setting requires_grad=False.
- It changes the behavior of layers like dropout and batch normalization for inference.
  - Explanation: Correct! eval() mode disables dropout and uses running statistics for batch norm instead of computing them from the current batch.
- It enables gradient computation for evaluation metrics.
  - Explanation: Actually, we typically use torch.no_grad() during evaluation to disable gradient computation.
- It automatically calculates evaluation metrics.
  - Explanation: model.eval() only changes layer behavior - you still need to implement metric calculation separately.

### 6. What is the purpose of `torch.no_grad()` during evaluation?

**Choices**
- To prevent the model from making predictions.
  - Explanation: torch.no_grad() doesn't prevent predictions, just gradient computation.
- To save memory and speed up computation by disabling gradient tracking.
  - Explanation: Correct! Since we don't need gradients for evaluation, disabling them saves memory and computation.
- To enable evaluation mode for the model.
  - Explanation: Evaluation mode is enabled with model.eval(), not torch.no_grad().
- To ensure consistent results across runs.
  - Explanation: Reproducibility is handled by setting random seeds, not torch.no_grad().

### 7. What changes when you use 🤗 Accelerate in your training loop?

**Choices**
- You must rewrite your entire training loop from scratch.
  - Explanation: Accelerate requires minimal changes to existing PyTorch code.
- You wrap key objects with accelerator.prepare() and use accelerator.backward() instead of loss.backward().
  - Explanation: Correct! These are the main changes - prepare your objects and use accelerator.backward() for proper distributed training.
- You need to specify the number of GPUs in your code.
  - Explanation: Accelerate automatically detects available hardware.
- You must use a different optimizer and scheduler.
  - Explanation: You can use the same optimizers and schedulers with Accelerate.
