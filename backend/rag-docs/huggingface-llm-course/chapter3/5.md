...
chapter: "chapter3"
original_path: "chapters/en/chapter3/5.mdx"
source_revision: "5805d51523d561a82520b301dbc8c5759b212844"
source_url: "https://github.com/huggingface/course/blob/5805d51523d561a82520b301dbc8c5759b212844/chapters/en/chapter3/5.mdx"
---
...
**Choices**
- The model is learning successfully and will continue to improve.
  - Explanation: If validation loss is increasing while training loss decreases, this indicates a problem, not success.
- The model is overfitting to the training data.
  - Explanation: Correct! This is a classic sign of overfitting - the model performs well on training data but poorly on unseen validation data.
- The learning rate is too low.
  - Explanation: A low learning rate would cause slow learning, not the divergence between training and validation performance.
- The dataset is too small.
  - Explanation: While small datasets can contribute to overfitting, this specific pattern is the definition of overfitting regardless of dataset size.

### 2. Why do accuracy curves often show a "steppy" or plateau-like pattern rather than smooth increases?

**Choices**
- There's an error in the accuracy calculation.
  - Explanation: The steppy pattern is normal and expected, not an error.
- Accuracy is a discrete metric that only changes when predictions cross decision boundaries.
  - Explanation: Correct! Unlike loss, accuracy depends on discrete prediction decisions, so small improvements in confidence may not change the final accuracy until a threshold is crossed.
- The model is not learning effectively.
  - Explanation: Steppy accuracy curves are normal even when the model is learning well.
- The batch size is too small.
  - Explanation: Batch size affects training stability but doesn't explain the inherently discrete nature of accuracy metrics.
...
- Increase the learning rate to speed up convergence.
  - Explanation: Increasing the learning rate would likely make the fluctuations worse.
- Reduce the learning rate and possibly increase the batch size.
  - Explanation: Correct! Lower learning rates and larger batch sizes typically lead to more stable training.
- Stop training immediately as the model won't improve.
  - Explanation: Erratic curves can often be fixed with hyperparameter adjustments.
- Switch to a completely different model architecture.
  - Explanation: This is premature - erratic curves are usually fixable with hyperparameter tuning.

### 4. When should you consider using early stopping?

**Choices**
- Always, as it prevents any form of overfitting.
  - Explanation: Early stopping is useful but not always necessary, especially if other regularization methods are working.
- When validation performance stops improving or starts degrading.
  - Explanation: Correct! Early stopping helps prevent overfitting by stopping training when the model no longer generalizes better.
- Only when training loss is still decreasing rapidly.
  - Explanation: If training loss is decreasing rapidly and validation performance is good, you might want to continue training.
- Never, as it prevents the model from reaching its full potential.
  - Explanation: Early stopping is a valuable technique that often improves final model performance by preventing overfitting.

### 5. What indicates that your model might be underfitting?

**Choices**
- Training accuracy is much higher than validation accuracy.
  - Explanation: This describes overfitting, not underfitting.
- Both training and validation performance are poor and plateau early.
  - Explanation: Correct! Underfitting occurs when the model lacks capacity to learn the patterns, resulting in poor performance on both training and validation data.
- The learning curves are very smooth with no fluctuations.
  - Explanation: Smooth curves are generally good and don't indicate underfitting.
- Validation loss is decreasing faster than training loss.
  - Explanation: This would actually be a positive sign, not a problem.
