...
chapter: "chapter3"
original_path: "chapters/en/chapter3/3.mdx"
source_revision: "5805d51523d561a82520b301dbc8c5759b212844"
source_url: "https://github.com/huggingface/course/blob/5805d51523d561a82520b301dbc8c5759b212844/chapters/en/chapter3/3.mdx"
---
# Fine-tuning a model with the Trainer API


[Course video](https://www.youtube.com/watch?v=nvBXf7s7vTI)
...
- It specifies which model architecture to use.
  - Explanation: Model architecture is specified when loading the model, not in the Trainer.
- It tells the Trainer which tokenizer to use for processing data.
  - Explanation: The processing_class parameter is a modern addition that helps the Trainer know which tokenizer to use.
- It determines the batch size for training.
  - Explanation: Batch size is set in TrainingArguments, not through processing_class.
- It controls the evaluation frequency.
  - Explanation: Evaluation frequency is controlled by eval_strategy in TrainingArguments.

### 2. Which TrainingArguments parameter controls how often evaluation occurs during training?

**Choices**
- eval_frequency
  - Explanation: There's no eval_frequency parameter in TrainingArguments.
- eval_strategy
  - Explanation: eval_strategy can be set to 'epoch', 'steps', or 'no' to control evaluation timing.
- evaluation_steps
  - Explanation: eval_steps sets the number of steps between evaluations, but eval_strategy determines if/when evaluation happens.
- do_eval
  - Explanation: There's no do_eval parameter in modern TrainingArguments.

### 3. What does <code>fp16=True</code> in TrainingArguments enable?

**Choices**
- 16-bit integer precision for faster training.
  - Explanation: fp16 refers to floating-point precision, not integer precision.
- Mixed precision training with 16-bit floating-point numbers for faster training and reduced memory usage.
  - Explanation: Mixed precision training uses 16-bit floats for forward pass and 32-bit for gradients, improving speed and reducing memory usage.
- Training for exactly 16 epochs.
  - Explanation: fp16 has nothing to do with the number of epochs.
- Using 16 GPUs for distributed training.
  - Explanation: The number of GPUs is not controlled by the fp16 parameter.

### 4. What is the role of the <code>compute_metrics</code> function in the Trainer?

**Choices**
- It calculates the loss during training.
  - Explanation: Loss calculation is handled automatically by the model, not by compute_metrics.
- It converts logits to predictions and calculates evaluation metrics like accuracy and F1.
  - Explanation: compute_metrics takes predictions and labels, then returns metrics for evaluation.
- It determines which optimizer to use.
  - Explanation: Optimizer selection is not handled by compute_metrics.
- It preprocesses the training data.
  - Explanation: Data preprocessing is done before training, not by compute_metrics during evaluation.

### 5. What happens when you don't provide an <code>eval_dataset</code> to the Trainer?

**Choices**
- Training will fail with an error.
  - Explanation: Training can proceed without an eval_dataset, though you won't get evaluation metrics.
- The Trainer will automatically split the training data for evaluation.
  - Explanation: The Trainer doesn't automatically create validation splits.
- You won't get evaluation metrics during training, but training will still work.
  - Explanation: Evaluation is optional - you can train without it, but you won't see validation metrics.
- The model will use the training data for evaluation.
  - Explanation: The Trainer won't automatically use training data for evaluation - it simply won't evaluate.

### 6. What is gradient accumulation and how do you enable it?

**Choices**
- It saves gradients to disk, enabled with save_gradients=True.
  - Explanation: Gradient accumulation doesn't involve saving gradients to disk.
- It accumulates gradients over multiple batches before updating, enabled with gradient_accumulation_steps.
  - Explanation: This allows you to simulate larger batch sizes by accumulating gradients over multiple forward passes.
- It speeds up gradient computation, enabled automatically with fp16.
  - Explanation: While fp16 can speed up training, gradient accumulation is a separate technique.
- It prevents gradient overflow, enabled with gradient_clipping=True.
  - Explanation: That describes gradient clipping, not gradient accumulation.
