...
chapter: "chapter12"
original_path: "chapters/en/chapter12/3.mdx"
source_revision: "5805d51523d561a82520b301dbc8c5759b212844"
source_url: "https://github.com/huggingface/course/blob/5805d51523d561a82520b301dbc8c5759b212844/chapters/en/chapter12/3.mdx"
---
...
[Embedded course media](https://reasoning-course-deepseek-ai-deepseek-r1-distill-0f5fad4.hf.space/)
...
**Choices**
- The GRPO algorithm that enables learning from preferences with and without a reward model
  - Explanation: Correct! GRPO's key innovation is its ability to directly optimize for preference rectification, making it more efficient than traditional RL methods.
- Using more GPUs for training than any previous model
  - Explanation: The paper's innovation is in its algorithmic approach (GRPO) rather than computational resources used.
- Creating a larger language model than existing ones
  - Explanation: The innovation lies in the training methodology and GRPO algorithm, not in model size.

### 2. What are the four phases of the DeepSeek R1 training process?

**Choices**
- Cold Start, Reasoning RL, Rejection Sampling, and Diverse RL
  - Explanation: Correct! These are the exact four phases described in the paper, each serving a specific purpose in the training pipeline.
- Pre-training, Fine-tuning, Testing, and Deployment
  - Explanation: The paper specifically outlines four different phases: Cold Start (quality foundation), Reasoning RL (capability building), Rejection Sampling (quality control), and Diverse RL (broad alignment).
- Data Collection, Model Training, Evaluation, and Optimization
  - Explanation: The paper describes a specific four-phase process that includes Cold Start, Reasoning RL, Rejection Sampling, and Diverse RL phases.

### 3. What is the 'Aha Moment' phenomenon in R1-Zero's training?

**Choices**
- A process where the model recognizes errors, self-corrects, and explains its corrections
  - Explanation: Correct! The paper describes this as a four-step process: initial attempt, recognition of errors, self-correction, and explanation of the improvement.
- The point where the model reaches human-level performance
  - Explanation: The 'Aha Moment' specifically refers to the model's ability to recognize and correct its own mistakes, similar to human problem-solving realizations.
- When the model completes its training process
  - Explanation: The 'Aha Moment' is about the model's emergent ability to recognize errors and self-correct during problem-solving, not about training completion.

### 4. How does GRPO's group formation work?

**Choices**
- It generates multiple solutions (4-16) for the same problem and evaluates them together
  - Explanation: Correct! GRPO generates multiple attempts at solving the same problem, typically 4, 8, or 16 different attempts, which are then evaluated as a group.
- It combines multiple models into one ensemble
  - Explanation: GRPO uses a single model to generate multiple solution attempts, not an ensemble of different models.
- It splits the training data into different groups
  - Explanation: GRPO's group formation involves generating multiple solutions for the same problem, not splitting training data.

### 5. What is the key difference between DeepSeek-R1-Zero and DeepSeek-R1?

**Choices**
- R1-Zero uses pure RL while R1 combines RL with supervised fine-tuning
  - Explanation: Correct! As shown in the comparison table, R1-Zero uses pure RL training while R1 uses a multi-phase approach combining supervised fine-tuning with RL, resulting in better language consistency.
- R1-Zero is smaller than R1
  - Explanation: The difference is in their training approaches (pure RL vs. multi-phase), not their model sizes.
- R1-Zero was trained on less data
  - Explanation: The key distinction is their training methodology: pure RL for R1-Zero versus a combined SFT and RL approach for R1.
